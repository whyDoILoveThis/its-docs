...
const Doc = ({ doc, refetchProjectForDocs, projUid, theProject }: Props) => {
  ...
  const handleUpdatePic = async (theUrl: string) => {
    ...
    setImgMsg("Deleting old image...");
    await fbDeleteImage(theUrl);
    setImgMsg("Uploading new image...");
    ...
    <div className="mb-24 flex flex-col items-center gap-2 w-full">
      ...
      {localDocItems &&
        localDocItems.map((item: DocItem, index) => (
          <div
            className={`relative z--ten mb-4 w-full max-w-[500px] `}
            key={index}
          >
            <div className="relative z--ten flex flex-col items-center">
              {item.style !== "code" &&
              item.style !== "pic" &&
              editDocItemIndex !== index ? (
                <div
                ...
                item.style === "pic" && (
                  ...
                  >
                    ...
                    <Image
                      className={`relative z-lowest transition-opacity duration-500 ${
                    ...
                    {imageUrl && item.text && (
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <Image
                          ...
                          src={imageUrl}
                          alt="originalImg"
                          onLoadingComplete={() => handleImgLoad(imageUrl)}
                        />
                      </div>
                    )}
                    {imageUrl && item.text && (

...
const Doc = ({ doc, refetchProjectForDocs, projUid, theProject }: Props) => {
  ...
  // Add new doc item
  const handleAddDocItem = async (image: File | null) => {
    try {
  ...
  return (
    <div className="mb-24 flex flex-col items-center gap-2 w-full">
      ...
      {(tempItemStyles.length > 0 || editMode) && (
        <div className="w-[65px] rounded-sm overflow-hidden bg-black bg-opacity-20 h-[25px] backdrop-blur-sm fixed place-self-start" />
      )}{" "}
